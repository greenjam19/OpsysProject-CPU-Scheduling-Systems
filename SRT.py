...
def SRT(num_procs, arr_time_p, CPU_bursts_p, IO_bursts_p, cont_switch_time, alpha_, tau_inits):
    ...
    # First print
    print("\ntime ", current_time,
          "ms: Simulator started for SRT [Q: empty]", sep="")
    ...
    while(True):
        ...
        if (len(arr_time) != 0):
            for i in range(len(arr_time)):
                ...
                if (current_time == arr_time[i]):
                    ...
                    if in_processor == True:
                        time_left = start_burst_time + \
                            tau[proc_idx] - current_time
                        if (preempt_times[proc_idx] != -1):
                            time_left = start_burst_time + \
                                tau[proc_idx] - current_time - \
                                (preempt_times[proc_idx] -
                                 CPU_bursts[proc_idx][0])
                        ...
                        if (tau[i] < time_left):
                            ...
                            if current_time < 1000:
                                print("time ", current_time, "ms: Process ", chr(
                                    65 + i), " (tau ", tau[i], "ms)", " arrived; preempting ", runn_proc, " [Q: ", sep='', end='')
                                print(*queue2, end='')
                                print("]")
                            num_preemps += 1
                            if (preempt_times[proc_idx] == -1):
            ...
            if (len(queue) == 0 and preempt_times[proc_idx] != -1):
                if current_time < 1000:
                    print("time ", current_time, "ms: Process ", current_proc, " (tau ",
                          tau[proc_idx], "ms)", " started using the CPU for remaining ", CPU_bursts[proc_idx][0], "ms of ", preempt_times[proc_idx], "ms burst [Q: empty]", sep='')
                ...
                if current_time < 1000:
                    print("time ", current_time, "ms: Process ", current_proc, " (tau ",
                          tau[proc_idx], "ms)", " started using the CPU for remaining ", CPU_bursts[proc_idx][0], "ms of ", preempt_times[proc_idx], "ms burst [Q: ", sep='', end='')
                    print(*queue, end='')
                    print("]")
                runn_proc = current_proc
                in_processor = True
        ...
        for i in range(len(wait_times)):
            ...
            # we were working in this area, there was a thing we were doing here !!!!
                if in_processor == True and len(queue) > 0 and len(CPU_bursts[proc_idx]) != 0:
                    ...
                    if (tau[i] < time_left and CPU_bursts[proc_idx][0] - (current_time - start_burst_time) >= 0):
                        # ? time 952ms: Process E (tau 40ms) completed I/O; preempting F [Q: E G A]
                        if current_time < 1000:
                            print("time ", current_time, "ms: Process ", chr(
                                65 + i), " (tau ", tau[i], "ms)", " completed I/O; preempting ", runn_proc, " [Q: ", sep='', end='')
                            print(*queue2, end='')
                            print("]")
                        num_preemps += 1
                        if (preempt_times[proc_idx] == -1):
    ...
    print("time ", current_time,
          "ms: Simulator ended for SRT [Q: empty]", sep='')

    avg = (sum(SRT_wait_times) - (cont_switch_time//2)*(SRT_cont_switches +
           num_preemps))/(sum([len(CPU_bursts_p[i]) for i in range(num_procs)]))
    SRT_CPU_util = math.ceil(
        sum([sum(x) for x in CPU_bursts_p])*100000/current_time)/1000
    avg = math.ceil(avg*1000)/1000
    """"""
    avg_turnaround = (sum(SRT_wait_times) + (cont_switch_time/2)*SRT_cont_switches-2*num_preemps)/(sum([len(CPU_bursts_p[i]) for i in range(
...
def main():
    ...
    if (False):
        ...
        SRT(num_procs, arr_time, CPU_bursts,
            IO_bursts, cont_switch, alpha, tau_inits)
